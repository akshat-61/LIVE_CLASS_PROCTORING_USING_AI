...
SEAT_ZONE_RADIUS         = 28    
SEAT_VACANCY_FRAMES      = 200
VACANCY_GRACE_FRAMES     = 12   
...
# FIX: 'paper' removed (now a detected class), 'person' added (skip from object model)
...
seat_grace_counters   = {}   # FIX 2: tracks how many consecutive frames a seat has been empty
seat_was_vacant       = {}
proximity_counters    = {}
...
def can_send(key, cooldown=None):
    ...
    if key not in last_alert_time or t - last_alert_time[key] > cd:
        last_alert_time[key] = t
        return True
    return False
...
def init_seat_zones(positions):
    ...
    seat_grace_counters = {sid: 0 for sid in seat_positions}   # FIX 2
...
def assign_seats_voronoi(current_positions):
    """
    FIX 3: Voronoi-style one-to-one seat assignment.

    For every YOLO-detected person, find their nearest seat center.
    Each seat can claim at most ONE detection (its closest).
    A seat is returned as occupied only if its winning detection is
    within MAX_VACANCY_DIST pixels.

    Returns dict[sid -> tid | None].
    """
    detection_to_seat = {}
    for tid, pos in current_positions.items():
...
def check_seat_zones(current_positions, frame):
    ...
    # FIX 3: single Voronoi pass — one dict tells us every seat's occupant (or None)
    occupancy = assign_seats_voronoi(current_positions)

    for sid, seat_pos in seat_positions.items():
        occupant = occupancy.get(sid)          # tid or None

        if occupant is not None:
            # Student is present — reset both counters
            seat_grace_counters[sid]   = 0     # FIX 2
            seat_vacancy_counters[sid] = 0     # FIX 1
            ...
            # FIX 2: grace window before vacancy counter starts climbing
            seat_grace_counters[sid] = seat_grace_counters.get(sid, 0) + 1

            if seat_grace_counters[sid] >= VACANCY_GRACE_FRAMES:
                # FIX 1: cap at threshold — never reset until student returns
                ...
                if seat_vacancy_counters[sid] >= SEAT_VACANCY_FRAMES:
                    if not seat_was_vacant.get(sid, False):
                        ...
                        if can_send(f"vacancy_{sid}", cooldown=SEAT_VACANCY_COOLDOWN):
                            logger.log_event(
                                student_id=sid,
                                event="SEAT_VACATED",
                                confidence=1.0,
        ...
        for j in range(i + 1, len(seat_ids)):
            ...
            if d < PROXIMITY_ONLY_DIST:
                ...
                if proximity_counters[pair_key] >= PROXIMITY_CONFIRM_FRAMES:
                    if can_send(f"prox_{pair_key[0]}_{pair_key[1]}"):
                        logger.log_event(
...
def run_ai_on_frame(frame: np.ndarray) -> np.ndarray:
    ...
    if last_custom_results is not None and last_custom_results.boxes is not None:
        for box in last_custom_results.boxes:
            ...
            if cls == CUSTOM_CLASS["PERSON"] and box.id is not None:
                ...
                current_ids.add(tid)
                sid = student_id_map.get(tid, "Scanning...")
    ...
    if not locked:
        ...
        if stable_counter >= STABILITY_FRAMES:
            ...
            # FIX (Bug 3): use _calib_accumulated_positions — face_based_positions is
            # empty at lock time since the mesh loop hasn't run yet.
    ...
    # ── POSE / BODY LEAN ──────────────────────────────────────────────────────
    # FIX (Bug 1): removed duplicate inner pose block that was overwriting sid/tid
        ...
        if sid is not None:
            ...
            if _settled and learner._stable_trigger(sid, "BODY_LEANING", leaning_detected):
                ...
                if can_send(f"lean_{sid}"):
                    ...
                    # FIX (Bug 5): score BODY_LEANING — was missing entirely
        ...
        for f_lms in mesh_res.multi_face_landmarks:
            ...
            if _settled and learner._stable_trigger(sid, "LOOKING_DOWN", result.looking_down):
                ...
                if can_send(f"down_{sid}"):
                    ...
                    # FIX (Bug 2): was incorrectly scoring PHONE_DETECTED here
            ...
            if _settled and learner._stable_trigger(sid, "LOOKING_LEFT", result.looking_left):
                ...
                if can_send(f"left_{sid}"):
                    ...
                    # FIX (Bug 4): score LOOKING_LEFT — was missing entirely
    ...
    # ── OBJECT DETECTION (trained model: calculator, paper, person) ───────────
    if object_results is not None and object_results.boxes is not None:
        for box in object_results.boxes:
            ...
            # FIX: per-class confidence thresholds + skip person class
            min_conf = OBJECT_CLASS_CONF.get(label, 0.60)
            if conf < min_conf:
                continue
            if label == "person":
                continue  # handled by person_model — never process here
            ...
            if label == "calculator":
                # Calculator gets its own high-priority alert
                if can_send(f"calc_{sid}"):
            ...
            # elif label not in OBJECT_LABEL_EXCLUSIONS:
            #     if can_send(f"object_{sid}_{label}"):
            #         save_evidence(frame, sid, "UNAUTHORISED_OBJECT")
            #         learner.register_event(sid, "UNAUTHORIZED_OBJECT")
            #         logger.log_event(
            #             student_id=sid,
            #             event=f"UNAUTHORIZED_OBJECT_{label}",
            #             confidence=conf,
            #             frame=frame_count
            #         )
            #         send_event_async("UNAUTHORIZED_OBJECT", sid)
            #         score = score_engine.add_event(sid, "UNAUTHORIZED_OBJECT")
            #         if score > 80:
            #             print(f"🚨 CHEATING HIGH RISK: {sid} | score={score}")
            #     current_alert = f"UNAUTHORIZED OBJECT: {sid} ({label})"
...
def process_video(cap, out):
    ...
    # FIX (Bug 6): blocking get with timeout — eliminates busy-wait CPU spin
    while True:
        try:
