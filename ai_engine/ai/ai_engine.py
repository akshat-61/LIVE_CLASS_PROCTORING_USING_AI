...
def get_gaze_direction(landmarks):
    ...
    looking_left  = (iris_gaze  < GAZE_LEFT_THRESHOLD)  or (head_ratio < 0.38)
    looking_right = (iris_gaze  > GAZE_RIGHT_THRESHOLD) or (head_ratio > 0.62)
    ...
    for i, h1 in enumerate(hands_data):
        for j, h2 in enumerate(hands_data):
            ...
            if dist(h1["wrist"], h2["wrist"]) < PASS_DIST_THRESHOLD:
                ...
                # FIX II: sorted pair key for PASSING_OBJECT too
                pair_key = f"pass_{min(sid1, sid2)}_{max(sid1, sid2)}"
                # FIX A (v9): skip if this pair already fired this frame
                if pair_key in fired_this_frame:
                    continue
                if can_send(pair_key, cooldown=30.0):  # FIX A: raised 15s→30s
    ...
    if last_custom_results is not None and last_custom_results.boxes is not None:
        for box in last_custom_results.boxes:
            ...
            if cls == CUSTOM_CLASS["PERSON"] and box.id is not None:
                ...
                # FIX C: reacquire lost tracking IDs by proximity to known seat centers
                    ...
                    if pos is not None:
                        ...
                        for s_sid, s_pos in seat_positions.items():
                            if s_sid in claimed_sids or s_sid == "S???":
                                continue
                            if dist(pos, s_pos) <= MAX_VACANCY_DIST:
...
# ---------------------------------------------------------------------------
# AIEngine — class wrapper so proctoring_engine can do AIEngine().analyze()
# ---------------------------------------------------------------------------
class AIEngine:
    """Thin class wrapper around the module-level analysis pipeline."""

    def analyze(self, features: dict) -> dict:
        """
        Run a lightweight risk score over the feature snapshot.
        Returns a dict with at least {"score": float, "flags": list}.
        """
        score = 0.0
        flags = []
        ...
        if head_pose and head_pose not in ("FORWARD", "CENTER", None):
            score += 20.0
            flags.append(f"head_pose:{head_pose}")

        # Object detection risk
        objects = features.get("objects_detected") or []
        if any(o for o in objects if "phone" in str(o).lower()):
            score += 40.0
            flags.append("phone_detected")
        if any(o for o in objects if "book" in str(o).lower()):
            score += 15.0
        ...
        if person_count == 0:
            ...
            score += 30.0 * (person_count - 1)
            flags.append(f"multiple_persons:{person_count}")

        # Gesture risk
        gesture = features.get("gesture")
        if gesture and gesture not in ("NONE", None):
            score += 10.0
            flags.append(f"gesture:{gesture}")

        score = min(score, 100.0)
        return {"score": round(score, 2), "flags": flags, "features": features}
